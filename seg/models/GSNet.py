...
class ResBlock(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv1 = nn.Sequential(
            ...
            norm_layer_1(n_ch2),
            nn.LeakyReLU(negative_slope=relu_slope),
            nn.Conv2d(n_ch2, n_ch2, kernel_size=3, stride=1, dilation=1, padding=1, padding_mode='reflect', bias=False),
            norm_layer_1(n_ch2),
            nn.LeakyReLU(negative_slope=relu_slope),
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(n_ch1, n_ch2, kernel_size=3, stride=1, dilation=2, padding=2, padding_mode='reflect', bias=False),
            norm_layer_1(n_ch2),
            nn.LeakyReLU(negative_slope=relu_slope),
            nn.Conv2d(n_ch2, n_ch2, kernel_size=3, stride=1, dilation=2, padding=2, padding_mode='reflect', bias=False),
            norm_layer_1(n_ch2),
            nn.LeakyReLU(negative_slope=relu_slope),
        )
...
class UpConv(nn.Module):
    def __init__(self, n_ch1, n_ch2):
        ...
        self.conv = nn.Sequential(
            nn.ConvTranspose2d(in_channels, mid_channels, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False),
            norm_layer_2(mid_channels),
            nn.ReLU(),
            nn.Conv2d(mid_channels, out_channels, kernel_size=3, stride=1, padding=1, padding_mode='reflect', bias=False),
            norm_layer_2(out_channels),
            nn.ReLU(),
        )
        self.cbam = CBAM(gate_channels=out_channels)
...
class Generator(nn.Module):
    def __init__(self, n_channels, n_classes, n_features=24, seg_net_out_opt=1):
        ...
        self.n_filters = n_features * np.array([1, 2, 4, 8, 16])
