...
class DRN(nn.Module):
    ...
    def __init__(self, in_channels, block, layers, arch='D',
...
class DRN_A(nn.Module):
    ...
    def __init__(self, in_channels, block, layers, BatchNorm=None):
        self.inplanes = 64
...
def drn_a_50(in_channels, BatchNorm, pretrained=True):
    model = DRN_A(in_channels, Bottleneck, [3, 4, 6, 3], BatchNorm=BatchNorm)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
    return model


def drn_c_26(in_channels, BatchNorm, pretrained=True):
    model = DRN(in_channels, BasicBlock, [1, 1, 2, 2, 2, 2, 1, 1], arch='C', BatchNorm=BatchNorm)
    if pretrained:
...
def drn_c_42(in_channels, BatchNorm, pretrained=True):
    model = DRN(in_channels, BasicBlock, [1, 1, 3, 4, 6, 3, 1, 1], arch='C', BatchNorm=BatchNorm)
    if pretrained:
...
def drn_c_58(in_channels, BatchNorm, pretrained=True):
    model = DRN(in_channels, Bottleneck, [1, 1, 3, 4, 6, 3, 1, 1], arch='C', BatchNorm=BatchNorm)
    if pretrained:
...
def drn_d_22(in_channels, BatchNorm, pretrained=True):
    model = DRN(in_channels, BasicBlock, [1, 1, 2, 2, 2, 2, 1, 1], arch='D', BatchNorm=BatchNorm)
    if pretrained:
...
def drn_d_24(in_channels, BatchNorm, pretrained=True):
    model = DRN(in_channels, BasicBlock, [1, 1, 2, 2, 2, 2, 2, 2], arch='D', BatchNorm=BatchNorm)
    if pretrained:
...
def drn_d_38(in_channels, BatchNorm, pretrained=True):
    model = DRN(in_channels, BasicBlock, [1, 1, 3, 4, 6, 3, 1, 1], arch='D', BatchNorm=BatchNorm)
    if pretrained:
...
def drn_d_40(in_channels, BatchNorm, pretrained=True):
    model = DRN(in_channels, BasicBlock, [1, 1, 3, 4, 6, 3, 2, 2], arch='D', BatchNorm=BatchNorm)
    if pretrained:
...
def drn_d_54(in_channels, BatchNorm, pretrained=True):
    model = DRN(in_channels, Bottleneck, [1, 1, 3, 4, 6, 3, 1, 1], arch='D', BatchNorm=BatchNorm)
    if pretrained:
...
def drn_d_105(in_channels, BatchNorm, pretrained=True):
    model = DRN(in_channels, Bottleneck, [1, 1, 3, 4, 23, 3, 1, 1], arch='D', BatchNorm=BatchNorm)
    if pretrained:
